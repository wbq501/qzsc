...
public class CommonUtil {
     ...
     * @return
     */
    public static boolean checkPhone(Context context,String phone, boolean toast) {
        if (TextUtils.isEmpty(phone)) {
            if (toast)
                BaseToast.ToastL(context,"手机号为空");
            return false;
        }
        if (phone.length() != 11 || !phone
                .matches("^((13)|(14)|(15)|(17)|(18))\\d{9}$")) {
            if (toast)
                BaseToast.ToastL(context,"手机号格式不对");
            return false;
        }
        return true;
    }
    ...
    public static boolean checkPhone(Context context,String phone) {
        ...
        return checkPhone(context,phone, true);
    }
    ...
    public static boolean isWeinxinAvilible(Context context) {
        final PackageManager packageManager = context.getPackageManager();
        List<PackageInfo> info = packageManager.getInstalledPackages(0);//获取已安装的程序包信息
        if (info != null) {
            for (int i = 0; i < info.size(); i++) {
                if (info.get(i).packageName.equals("com.tencent.mm")) {
                    return true;
                }
            }
        }
