...
public class MaterialDialog {
    ...
    private Button mPositiveButton,singleButton;
    ...
    private int pId = -1, nId = -1,sId=-1;
    private String pText, nText,sText;
    View.OnClickListener pListener, nListener,singleListener;
    ...
    public MaterialDialog setGravity(int gri) {

        if (mBuilder != null) {
            mBuilder.setGravity(gri);
        }
        return this;
    }

    public MaterialDialog setMargine(int left, int top, int right, int bottom) {

        if (mBuilder != null) {
            mBuilder.setMargins(left, top, right, bottom);
        }
        return this;
    }
    ...
    /**
     * 设置只显示单个按钮 自动隐藏掉两个按钮的
     * @param text 文字
     * @param mNListener
     * @return
     */
    public MaterialDialog setSingleButton(String text, View.OnClickListener mNListener) {
        this.singleListener=mNListener;
        sText=text;
        return this;
    }
    public MaterialDialog setSingleButton(int  resId, View.OnClickListener mNListener) {
        this.singleListener=mNListener;
        sId=resId;
        return this;
    }
    ...
    private class Builder {
        ...
        private Builder() {
            ...
            singleButton= (Button) mButtonLayout.findViewById(R.id.btn_mid);
            mNegativeButton = (Button) mButtonLayout.findViewById(R.id.btn_n);
            rootView= (LinearLayout) mAlertDialogWindow.findViewById(R.id.rootview);
            ...
            if (sId!=-1||!isNullOrEmpty(sText)){
                singleButton.setVisibility(View.VISIBLE);
                if (sId!=-1) {
                    singleButton.setText(nId);
                }else {
                    singleButton.setText(sText);
                }
                singleButton.setOnClickListener(singleListener);
                if (isLollipop()){
                    singleButton.setElevation(0);
                }
            }
